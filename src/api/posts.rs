...
pub async fn get_download_link(state: &AppState, filepath: &str) -> Option<String> {
	let command = tokio::process::Command::new("rclone")
		.arg("link")
		.arg(format!("{}/{}", state.config.rclone_path, filepath))
...
pub async fn continue_pending_upload_ws(mut socket: ws::WebSocket, state: AppState) {
	...
	for file in &files {
		loop {
			let Some(download) = get_download_link(&state, file).await else {
...
pub async fn delete_post(
	...
	for file in post.local_files {
		_ = tokio::process::Command::new("rclone")
			.arg("delete")
			.arg(format!("{}/{}", state.config.rclone_path, file))
			.arg("--config=/etc/rclone-mnt.conf")
			.output()
			.await;
	}
